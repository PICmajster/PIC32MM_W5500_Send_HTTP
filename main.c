...
uint8_t POST_Ubidots[256] ; //buffor for HTTP Request
uint8_t ile ;
...
sprintf(POST_Ubidots,"{\"temperaturasalon\": %d}",licznik);
ile = strlen(POST_Ubidots); //how many characters a data string

/*HTTP Request*/
sprintf(POST_Ubidots,\
"POST /api/v1.6/devices/pic32 HTTP/1.1\r\n\
Host: industrial.api.ubidots.com\r\n\
X-Auth-Token: BBFF-dlkCOCzYMfLY9AV5wgxgsDrP2brXYZ\r\n\
Content-Type: application/json\r\n\
Content-Length: %d\r\n\r\n\
{\"temperaturasalon\": %d}\
\r\n",ile,licznik);    // licznik = 1...10
    ...
    switch(getSn_SR(SOCK_ID_TCP))
    {
        ...
        case SOCK_INIT :
        {
              ...
              if( (ret = connect(SOCK_ID_TCP,IP_ubidots, 80)) == SOCK_OK)
            {
                   return;
            }
